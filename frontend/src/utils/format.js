...
/** 게임의 현재 날짜/연도 기준 YYYY -> 'n년 전/후' (같은 해는 '0년 전') */
...
export const relativeYearDate = (value, referenceDateOrYear) => {
  ...
  return `${relativeYearLabel(match[1], referenceDateOrYear)} ${match[2].padStart(2, '0')}/${match[3].padStart(2, '0')}`;
};
...
export const relativeDateValue = (value, referenceDateOrYear) => {
  ...
  if (yearMonth) return `${relativeYearLabel(yearMonth[1], referenceDateOrYear)} ${yearMonth[2].padStart(2, '0')}월`;
