...
module.exports = {
  ...
  // --- 코인 거래 단위 (§9-3: 코인은 소수 허용하되 부동소수 노이즈 방지 위한 상한 필요) ---
  // 최소 거래 수량(이 미만 거래 불가) / 소수 자릿수 상한. 극소수량(예: 1e-18) 거래를 막아
  // 평가액 계산(가격 × 수량)에 부동소수 오차가 끼는 것을 방지한다.
  COIN_MIN_TRADE_UNIT: 0.0001,
  COIN_MAX_DECIMALS: 4,

  // 세션 층화추출 후보의 시작 시점 종가 하한 (coinUniverseService).
  // 원 단위 정수 현금 체계라 저가 코인은 두 가지 문제가 있다:
  //   1) 1원 미만은 매수/매도 금액이 0원으로 뭉개져 거래 자체가 성립하지 않는다
  //      (실측: 8.5e-7원 코인 100개 매수 1원 / 매도 0원)
  //   2) 1~99원대는 거래는 되지만 가격 해상도가 없다 - 1원이 2원이 되면 +100%다
  // 그래서 하한을 100원으로 둔다. 이 값을 올리면 특히 small 티어 후보가 줄어드니
  // 조정 시 초기 연도(2014~2016) 후보 수를 함께 확인할 것.
  COIN_MIN_PRICE_KRW: 100,
  ...
  // --- 데이터 기간 ---
  // 적재된 데이터의 전체 범위. 2013년치는 게임 시작 구간이 아니라 **룩백 버퍼**다 —
  // 어느 시점에서 플레이해도 1년 전 뉴스/시세를 조회할 수 있게 하려고 일부러 넣었다.
  DATA_RANGE: { from: '2013-01-02', to: '2023-12-31' },

  // 게임 시작일이 뽑히는 구간 (turnSelector.selectTurnDates).
  // DATA_RANGE보다 1년 늦게 시작한다 = 2013년 룩백 버퍼를 시작일 후보에서 제외한다.
  // 코인 후보 수 측면에서도 2013년은 유니버스에 2종밖에 없어 게임이 성립하지 않는다.
  // 환경변수 GAME_START_RANGE가 있으면 그쪽이 우선한다(.env.example 참고).
  START_RANGE: { from: '2014-01-02', to: '2023-12-31' },
};
