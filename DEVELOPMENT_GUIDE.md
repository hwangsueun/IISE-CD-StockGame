...
| 이벤트 A~E (독촉전화/급등주/스터디/경조사/명절/기절) | ✅ 완료·검증 | 명절 달력·스터디 힌트 실데이터만 TODO |
...
| A6 | 명절(설/추석) 실제 달력 | `constants.js` HOLIDAY에 날짜 목록 추가 → `eventEngine.holiday` 트리거 교체 | data |
...
| B5 | 기절 중 월말 경과 시 "해당 월 미납 처리" 자동화 (미팅5 명세) — 현재는 상환 API를 안 부르면 기록이 안 남음 → 상환 턴을 지나치면 자동 미납(ratio 0) 기록 | `turnService.advanceTurn`에 상환 턴 통과 검사 추가 |
...
### Phase D — 프론트 디자인/연출 (Phase A~B와 병행 가능)

> 목표: 기능 확인용 UI → 디자인 시안(이미지 보드) 반영.
> 완료 기준: 시안 대비 화면 리뷰 통과. 기능 회귀 없음.

| # | 작업 | 지점 |
|---|---|---|
| D1 | 방(메인) 화면 배경/오브젝트 버튼 — 기존 `stock-game/src/assets/images`의 room.png 등 재활용 가능 | `MainPage`, `global.css` |
| D2 | 오프닝 컷신/일러스트 | `OpeningPage` STORY 배열 |
| D3 | 캘린더 월 그리드 UI (현재 날짜 선택 폼) | `CalendarModal` |
| D4 | 캔들차트/거래량 바 (현재 라인차트+지표) | `PriceChart` |
| D5 | 엔딩 월별 자산 추이 차트 + AI 분석 표시 | `ResultPage` (데이터는 이미 응답에 있음) |
| D6 | 이벤트별 연출 (독촉전화 대화 연출 — 미팅5 "압박형은 대화 연출 포함") | `EventPopup` |
| D7 | 사운드/BGM (선택) | 신규 |
...
## 5. 현재 열려 있는 TODO 전체 목록 (2026-07-07 grep 기준)

**data (8)** — Phase A 대응: 마스킹 사전 2건(`maskingService`), 갤러리 매핑(`import_community`), npq 수급(`import_stocks`), masked_name/섹터(`import_stocks`), 반기 재무(`import_all`), 명절 달력(`constants`/`eventEngine`), 코인 티어(`import_coins`).
